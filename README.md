...
Currently there are 2 implementations available, "Unified Evaluation" that consists on a modified script of `lm-evaluation-harness` that allows to run evaluations of the three tasks in a dependent way and "Harness Evaluation" that consists of regular tasks implementation using `lm-evaluation-harness`.
...
3. **Install the dependencies:**
    ...
    To use "Harness Evaluation":

    Follow the instalation guide of the official Calamita repo (https://github.com/CALAMITA-AILC/calamita-eval)
...
Use the standard `lm-evaluation-harness` command to run the evaluation. The tasks are defined in a way that allows running evaluations of the three tasks in a dependent sequence. The models are evaluated on instances that were only guessed correctly in the previous tasks. Unlike in "Unified Evaluation," this implementation requires executing the evaluation 3 times, once with each task, in order: Story Classification, Conflict Detection, and Physical State. Temporary files are created during the evaluations to pass information between the tasks; after all three evaluations have been completed, these files can be removed.

We provide an `eval.sh` bash script to simplify the evaluation pipeline. `eval.sh` was designed to use with base models; to evaluate instructed models with a chat template, use `eval-instruct.sh`.

An example command to evaluate the model `Meta-Llama-3.1-8B-Instruct` is included in the `Llama3_1-8b-Instruct.slurm` file.
...
GITA4CALAMITA_PREFIX="Llama-3.1-8B-Instruct" # Prefix for the temporary files to allow parallel evaluations
...
*** These results were calculated using `lm-evaluation-harness` v0.4.0; the usage of newer versions may result in different metric values, especially versions after v0.4.6.
...
View the results:
    - The results will be saved in the `results/` directory.
    - You can view the results in the JSON files generated by `lm-evaluation-harness`

```json
"gita_story_class": {
    "alias": "gita_story_class",
    "accuracy,none": 0.7275280898876404,
    "accuracy_stderr,none": "N/A",
    "accuracy_cloze,none": 0.8205128205128205,
    "accuracy_cloze_stderr,none": "N/A",
    "accuracy_order,none": 0.6475409836065574,
    "accuracy_order_stderr,none": "N/A",
    "accuracy_plausible,none": 0.717948717948718,
    "accuracy_plausible_stderr,none": "N/A"
}
```
```json
    "gita_conflict_detect": {
      "alias": "gita_conflict_detect",
      "consistency,none": 0.28870292887029286,
      "consistency_stderr,none": "N/A",
      "consistency_cloze,none": 0.39316239316239315,
      "consistency_cloze_stderr,none": "N/A",
      "consistency_order,none": 0.1885245901639344,
      "consistency_order_stderr,none": "N/A"
    }
```
```json
    "gita_physical_state": {
      "alias": "gita_physical_state",
      "verifiability,none": 0.14644351464435146,
      "verifiability_stderr,none": "N/A",
      "verifiability_cloze,none": 0.18803418803418803,
      "verifiability_cloze_stderr,none": "N/A",
      "verifiability_order,none": 0.10655737704918032,
      "verifiability_order_stderr,none": "N/A"
    }
```

Harness is not able to calculate the `stderr` of the metrics due to our special implementation of the tasks, so it always prints "N/A".